...
montageA = np.zeros((720 * 5, 1280, 3), dtype = "uint8")
montageB = np.zeros((720 * 5, 1280, 3), dtype = "uint8")
...
for j in xrange(0, 10):
	...
	if j < 5:
		montageA[j * 720:(j + 1) * 720, :] = result

	# otherwise, the second montage should be used
	else:
		montageB[(j - 5) * 720:((j - 5) + 1) * 720, :] = result

# show the results
cv2.imshow("Results 1-5", montageA)
cv2.imshow("Results 6-10", montageB)
cv2.imwrite("A.jpg",montageA)
cv2.imwrite("B.jpg",montageB)
cv2.waitKey(0)